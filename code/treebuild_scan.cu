...
__device__ unsigned int n_completed = 0;
...
#pragma unroll
    ...
    if (threadIdx.x == 0)
    {
      ...
      if (err != cudaSuccess)
      {
        printf(" launch failed 2: %s  level= %d n =%d n_run= %d\n", cudaGetErrorString(err), level, n_in_que, n_running);
        assert(0);
      }
#endif
#endif
...
#pragma unroll
    ...
    if (threadIdx.x == 0)
    {
      ...
      unsigned int res = atomicExch(&octCounterBase[5],1);
#if 1
      int waitCounter = 0;
      volatile int &lastBlock = shmem[0];
      while (res != 0)
      {
        for (int i = 0; i < 102; i++)
          waitCounter++;
        lastBlock = waitCounter;
        res = atomicExch(&octCounterBase[5],1);
      }
#endif
      ...
      {
        const cudaError_t err = cudaGetLastError();
        printf(" launch failed 1: %s  level= %d n =%d n_run= %d \n", cudaGetErrorString(err), level, n_in_que, n_scheduled-n_completed);
        assert(0);
      }
        cudaDeviceSynchronize();
       atomicExch(&octCounterBase[5],0);
#if 0
      int cntr  = 0;
      cudaError_t err = cudaGetLastError();
      while (err == cudaErrorLaunchPendingCountExceeded) //!= cudaSuccess)
      {
        cntr++;
        //assert(cntr < 10000);
        cudaDeviceSynchronize();
...
void testTree(const int n, const unsigned int seed)
{
  ...
  cudaDeviceSetLimit(cudaLimitDevRuntimePendingLaunchCount,204800); //1024*1024); 
