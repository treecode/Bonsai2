...
template<int NPERT, int NLEAF, typename T, bool STOREIDX>
    ...
    const int level = 0)
{
  ...
  /* process particle array */
  const int nBeg_block = nBeg + NPERT*blockIdx.x * BLOCKDIM;
#ifdef IOCOUNT
  if (threadIdx.x == 0 && blockIdx.x == 0)
    atomicAdd(&io_words, (nEnd-nBeg)*4*sizeof(T4)/sizeof(float4));
  int nio_per_warp = 0;
#endif
  for (int i = nBeg_block; i < nEnd; i += NPERT*gridDim.x * BLOCKDIM)
  {
    Particle4<T> p4list[NPERT];
#pragma unroll
    for (int kk = 0; kk < NPERT; kk++)
      p4list[kk] = ptcl[min(i+threadIdx.x+kk*BLOCKDIM,nEnd-1)];
#pragma unroll
    for (int kk = 0; kk < NPERT; kk++)
    {
...
#pragma unroll
      for (int k = 0; k < 8; k++)  /* process particles in shared memory */
      {
        if (i + (k<<WARP_SIZE2) + kk*BLOCKDIM >= nEnd) break;
        const int locid = (k<<WARP_SIZE2) + laneIdx;
        const int  addr = i + kk*BLOCKDIM+ locid;
        //      if (addr >= nEnd + laneIdx) break;
        const bool mask = addr < nEnd;
...
#endif
      {
        buildOctant<NPERT,NLEAF,T,false><<<grid,block,0,stream>>>
          (box, cellIndexBase+blockIdx.y, cellFirstChildIndex,
           octant_mask, octCounterNbase, buff, ptcl, level+1);
      }
    }
...
#pragma unroll
  ...
  cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);
#if 1
  buildOctant<NPERT, NLEAF,T,true><<<grid, block,0,stream>>>
...
void testTree(const int n, const unsigned int seed)
{
  ...
  CUDA_SAFE_CALL(cudaFuncSetCacheConfig(&buildOctant      <NPERT,NLEAF,T,true>, cudaFuncCachePreferShared));
  CUDA_SAFE_CALL(cudaFuncSetCacheConfig(&buildOctant      <NPERT,NLEAF,T,false>, cudaFuncCachePreferShared));
...
int main(int argc, char * argv [])
{
  ...
  const  int NPERT = 4;
#else
  const int NPERT = NPERTHREAD;
#endif


  testTree<NPERT, NLEAF, real>(n, argc > 2 ? atoi(argv[2]) : 19810614);
